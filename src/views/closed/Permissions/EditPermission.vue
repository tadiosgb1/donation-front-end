    ...
    <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-pop-in border border-slate-100">
      ...
      <div class="p-8">
        <div class="flex justify-between items-start mb-8">
          <div>
            <h2 class="text-2xl font-black text-slate-900 leading-none">Edit Permission</h2>
            <p class="text-[10px] font-black uppercase tracking-[0.2em] text-primary mt-2 flex items-center gap-2">
              <i class="fas fa-fingerprint"></i> Update System Identifier
            </p>
          </div>
        ...
        <form @submit.prevent="submitForm" class="space-y-6">
          <div class="space-y-2">
            <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">System Key (identifier)</label>
            <div class="relative group">
              <i class="fas fa-terminal absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-dprimary transition-colors text-xs"></i>
              <input 
                v-model="form.key" 
                type="text" 
                required 
                class="w-full pl-11 pr-4 py-3.5 bg-slate-50 border-2 border-slate-50 rounded-2xl focus:bg-white focus:ring-0 focus:border-primary outline-none transition-all text-sm font-bold text-slate-800 font-mono" 
              />
            </div>
          </div>

          <div class="space-y-2">
            <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Purpose Description</label>
            <div class="relative group">
              <i class="fas fa-info-circle absolute left-4 top-4 text-slate-300 group-focus-within:text-dprimary transition-colors text-xs"></i>
              <textarea 
          ...
          <div class="flex gap-3 pt-4">
            ...
            >
              <template v-if="!loading">
                Apply Changes <i class="fas fa-check text-[8px] group-hover:scale-125 transition-transform"></i>
              </template>
              <template v-else>
...
export default {
  ...
  methods: {
    async submitForm() {
      this.loading = true;
      try {
        const res = await this.$apiPut("/permission", this.data.id, this.form);
        if (res) {
          this.$root.$refs.toast.showToast('System Key Updated Successfully', 'success');
          this.$emit("saved");
          this.$emit("close");
        }
      } catch (e) { 
        console.error(e);
        this.$root.$refs.toast.showToast('Failed to update system key', 'error');
      } finally {
        this.loading = false;
      }
    }
