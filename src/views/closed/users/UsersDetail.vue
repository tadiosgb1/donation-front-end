...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800">
    ...
    <div class="bg-white overflow-hidden rounded-md border border-gray-200 p-4 hidden md:block space-y-2">
      ...
      <div><strong>Name:</strong> {{ item.name }}</div><div><strong>Email:</strong> {{ item.email }}</div><div><strong>Password:</strong> {{ item.password }}</div><div><strong>Confirm_password:</strong> {{ item.confirm_password }}</div>
    </div>
