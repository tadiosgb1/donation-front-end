  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 text-sm">
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div>
          <label class="block mb-1 text-sm font-medium text-gray-700">Confirm_password</label>
          <input v-model="form.confirm_password" type="text" required class="border border-gray-300 rounded-lg px-4 py-2 text-sm w-full sm:max-w-xs focus:outline-none focus:ring-2 focus:ring-green-500 shadow-sm transition duration-150" />
        </div>
